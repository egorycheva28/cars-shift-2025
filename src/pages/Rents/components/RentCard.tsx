...
const RentCard: React.FC<CarCardProps> = ({ rent }) => {
    ...
    return (
        ...
        >
            <div style={{ display: 'flex', gap: '2px', width: '150px' }}>
                <span style={{ fontSize: '16px' }}>{rent.carInfo.name}</span>
            </div>
            <div style={{ display: 'flex', gap: '8px', width: '428px' }}>
                <span style={{ fontSize: '16px' }}>{formatTimestampToDate(rent.startDate)} - {formatTimestampToDate(rent.endDate)}</span>
            </div>
            <div style={{ display: 'flex', gap: '12px', flexDirection: 'row', width: '160px', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: '16px' }}>{activeCar(rent.status, rent.startDate, rent.endDate) ? 'Активна' : translateStatus[rent.status]}</span>
            </div>
